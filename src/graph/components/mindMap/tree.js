...
export default class Tree extends Base {
  ...
  nodeDistance(d) {
    ...
    return d.depth
      ? this.nodeDistance(d.parent) + this.nodeWidth(d.parent) * 0.5 + width
      : 0;
  }
  ...
  renderNodes() {
    ...
    let enter = node
      ...
      .append("g")
      .attr("class", (d) =>
        `node node-${this.css} node-${this.classify(d)} ${
          d.data.type || ""
        }`.trim()
      )
      .attr("id", (d) => d.id)
      ...
      .attr("opacity", 0)
      .on("click", (d, i, nodeArr) =>
        this.parent.onTreeNodeClick(d, this, nodeArr[i].firstChild)
      )
      ...
      case 2:
        d3.select(`#${d.id}`)
          ...
          .attr(
            ...
            this.direction > 0
              ? 0
              : this.direction * (textLength * FONTSIZE + PADDING * 2)
          )
        ...
        d3.select(`#${d.id}`)
          ...
          .attr(
            "transform",
            `translate(${
              this.direction * ((textLength * FONTSIZE) / 2 + PADDING)
            },0)`
          )
          .append("text")
          .append("tspan")
  ...
  renderMarkers() {
    let marker = this.g
        .selectAll(`.marker-${this.css}`)
        .data(this.markers, (d) => d.id),
      enter = marker
        .enter()
        .append("polygon")
