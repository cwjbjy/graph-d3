...
function read_Element(ParentNode, OrigData) {
  ...
  for (let cd = 0, len = Children.length; cd < len; cd++) {
    ...
    if (TagName === "text")
      Child.setAttribute("dy", parseFloat(Child.getAttribute("dy") || 0) + 5);
  }
}

export default function downloadPNG(moduleName, svgDom, pngName) {
  ...
  let newChart = newSvgNode.querySelector(".mind"),
    ...
    mindTArr = mindTransform
      .replace(
        /translate\((-?[0-9.]*)[^0-9-]*(-?[0-9.]*)\D*([0-9.]*)/g,
        "$1 $2 $3 "
      )
      .split(" "),
  ...
  actualMap.setAttribute(
    "transform",
    `translate(${mX}, ${mY}) scale(${needScale})`
  );
  //源节点属性
  ...
  const bgHeight = Math.max(
    graphExportFooterHeight * k,
    graphExportFooterHeight
  );
  const marginY = 50;
  //新图的基本属性
  ...
  graphImg.onload = function () {
    ...
    ctx.fillRect(
      0,
      newCanvasBasicAttr.h - bgHeight,
      newCanvasBasicAttr.w,
      bgHeight
    );
